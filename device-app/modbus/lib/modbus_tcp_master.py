...
def read_modbus_tcp_data(tag,  modbus_response_deque, tcphandler):     
    ...
    if modbusFunction == "01-ReadCoils":
        ...
        if count is not False:    
            ...
            response = tcphandler.tcp_connection.read_coils(address,count,unit=1)
            logger.info("[RECV] Modbus TCP respone FC:{} {}: {}".format(modbusFunction, commandName, response.bits[0]))
    ...
    if modbusFunction == "02-ReadDiscreteInputs":
        try:
            ...
            response = tcphandler.tcp_connection.read_discrete_inputs(address, count, unit=1)
            logger.info("[RECV] Modbus TCP respone FC:{} {}: {}".format(modbusFunction, commandName, response.bits[0]))
    ...
    if modbusFunction == "03-ReadHoldingRegisters":
        ...
        if count is not False:  
            try:
                ...
                response = tcphandler.tcp_connection.read_holding_registers(address, count,unit=1)
                # return list of registers with size 1 
                logger.info("[RECV] Modbus TCP respone FC:{} {}: {}".format(modbusFunction, commandName, response.registers[0]))
    ...
    if modbusFunction == "04-ReadInputRegisters":  
        ...
        if count is not False:  
            ...
            response = tcphandler.tcp_connection.read_input_registers(address,count,unit=1) 
...
def read_modbus_tcp_loop(modbus_tags, connection_status_tcp, slave_ip, polling_interval, modbus_response_deque, tcphandler):
    try:
        while True:
            for tag in modbus_tags:
                ...
                if tag["enable"] == True: 
                    if connection_status_tcp is True:
                        logger.debug("[OPEN] Modbus TCP Master Connected with Slave IP: {}".format(slave_ip))
                        read_modbus_tcp_data(tag,  modbus_response_deque, tcphandler)
                        logger.debug("[SEND] Modbus TCP request to Slave IP: {}".format(slave_ip))
                    else:
